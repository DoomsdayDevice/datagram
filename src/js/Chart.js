...
export default class Chart{
  constructor(data, title){
    ...
    this.drawMinimap();
    this.drawNumbers(this.configureParametersForGraph().ceiling, 1, NUM_OF_FRAMES);
  ...
  drawDates({xStart, xEnd, xOffset, xEndPoint, xStartPoint}) {
    ...
    this.iCtx.clearRect(0, this.graph.height - DATE_SPACE, this.graph.width, DATE_SPACE);
  ...
  drawNumbers(ceiling, newOldRelationship = 1, currentFrame = 1, side="L") {
    ...
    // let distributedDifference = difference / NUM_OF_FRAMES;
    let rowHeight = (this.graph.height - DATE_SPACE) / NUM_OF_ROWS * distributedRelationship * currentFrame;
    // TODO round floats
  ...
  displayTooltip(currentArrayColumn, currentXPos){
    ...
    date = DAYS_OF_WEEK[date.getDay()] + ", " + MONTHS[date.getMonth()] + ' ' + date.getDate();
    let color;
